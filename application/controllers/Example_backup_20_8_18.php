...
class Example extends CI_Controller {
	...
	private function downloadAlbum($albumId)
	{
		...
		$album = $this->getAllImage($albumId);
		...
		if (!file_exists($path)) {
			...
			{
				$this->session->set_userdata("dcount",$i);
	...
	public function downloadSingle()
	{
		...
		$data = $this->downloadAlbum($albumId);
	...
	{
		$myfile = fopen("albums/".$this->session->userdata("userid")."/downloadcount.txt", "r") or die("Unable to open file!");
		$dcount = fread($myfile,filesize("albums/".$this->session->userdata("userid")."/downloadcount.txt"));
		fclose($myfile);
		echo $dcount;
	}
